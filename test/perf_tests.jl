...
@testset "Performance Test" begin
    ...
    @testset "Perf allocations" begin
        ...
        println("  • label: $(@allocated label(ctx, "Ceci est un simple texte sur une ligne.")) bytes")
        println("  • text: $(@allocated text(ctx, "Ceci est un\n long texte qui\n sera sur plusieurs\n lignes.")) bytes")
        println("  • button: $(@allocated button(ctx, "Test")) bytes")
        println("  • checkbox!: $(@allocated checkbox!(ctx, "Checkbox", Ref(false))) bytes")
        println("  • textbox!: $(@allocated textbox!(ctx, Ref("Texte dans une Textbox"), 100)) bytes")
        println("  • slider!: $(@allocated slider!(ctx, Ref(5.0f0), 0.0f0, 10.0f0)) bytes")
        println("  • number!: $(@allocated number!(ctx, Ref(42.0f0), 1.0f0)) bytes")
